...
Z1 = np.arange(0.6, 0.92, 0.05).tolist()
M1 = 1.5
for k in range(0,len(Z1),1):
    print("---------------Z1------------:", Z1[k])
    T1_list =np.linspace(700-150*Z1[k], 640 , 6)
    ...
    for j in range(0,len(T1_list),1):
        ...
        p2 = np.linspace(P1*1.5,P1*(2.0 + 0.3*k/3) ,1000) # post-shock pressure 
        p2 = pd.Series(p2)
        u2 = np.zeros(p2.size) 
        d2 = np.zeros(p2.size) 
