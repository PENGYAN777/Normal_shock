...
def PGfromZT(Z,T):
    ...
    Prange = np.linspace(Pc/10,Pc*2,1000)
...
def PTfromZG(Z,Gamma):
    ...
    Trange = np.linspace(500,Tmax,100)
    Trange = pd.Series(Trange)
    Prange = np.zeros(100)
    Gammat = np.zeros(100)
    # loop for P,T
...
def ZPfromTG(T,Gamma):
    ...
    Prange = np.linspace(Pc/5,Pc*2,500)
    Prange = pd.Series(Prange)
